...
export type AuthPageInitialSession = {
  user?: {
    email?: string | null;
  } | null;
} | null;
...
export default function AuthPageClient({
  ...
  initialSession: AuthPageInitialSession;
  ...
  const isAuthenticated = isSessionPending
    ? Boolean(initialSession?.user)
    : Boolean(clientSession?.user);
  const signedInEmail = isSessionPending
    ? initialSession?.user?.email
    : clientSession?.user?.email;
  const signedInEmailLabel = signedInEmail || "your account";
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-12 text-slate-900">
      <div className="mx-auto w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {isAuthenticated && !isResetPassword ? (
          <div className="space-y-4">
            <p className="text-sm text-slate-700">
              Signed in as{" "}
              <span className="font-medium">{signedInEmailLabel}</span>.
            </p>
            <button
